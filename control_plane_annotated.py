...
import iForest_detect           # import iForest_detect.py
import Kitsune.KitNET as kit
from load_data import *         # imoprt load_data.py
from model import Magnifier     # import Magnifier from AE.py
import warnings
...
# could be improved
# create the folders to save the output in
...
# setup the random seeds used to ensure reproducibility
def setup_seed(seed):
...
# Processes data from a DataFrame for model training or evaluation.
def data_processing(df, TWO_D):  # for testing
    # Determine the path to the scaler file based on KITSUNE flag and OPEN_SOURCE flag
    if KITSUNE:
    ...
    # Extract features (drop 'class' and '0' columns) and labels from the DataFrame
    X, y = df.drop(columns=['class', 0]), df['class']  # 0 is for hash key
    # Convert features and labels to NumPy arrays
    X, y = X.values, y.values

    # Apply the loaded scaler to transform the features
    X = scaler.transform(X)
    
    # Additional processing only if not using KITSUNE model
    if not KITSUNE:
        if TWO_D:
            # Pad features with zeros on both sides along the second dimension (columns) by 3 units
            X = np.pad(X, ((0,0),(3,0)), 'constant')
            index = np.empty((0,0))
            # Define sub-indices for different feature groups (e.g., Port statistics, MI statistics)
            ...
            # Loop through each sub-index and append them to the main index array
            for i in range(5):
            ...
            # Select specific features based on the index and reshape for 3D format
            # Commenting out the original line assuming a different output shape is desired
            # X = X[:, index.astype(np.int).tolist()].reshape(-1, 1, 5, 20)
            X = X[:, index.astype(np.int).tolist()].reshape(-1, 5, 21)
        else:
            # Add a new channel dimension at the second position (axis 1) for non-2D data
            X = X[:, np.newaxis, :]
        # Convert processed features and labels to PyTorch tensors
        X = torch.tensor(X, dtype=torch.float32)
        y = torch.tensor(y)
    return X, y
 
# Prepares and processes data for the Kitsune model.
def train_data_processing_Kitsune(df_normal_train, df_attack_eval):
    ...
    # Split the normal training data into training and validation sets
    ...
    # Return the scaled features for training and evaluation
    return X_train, X_normal_eval, X_attack_eval

# Prepares and processes data for training and evaluation.
def train_data_processing(df_normal_train, df_attack_eval, TWO_D):
    # data preparing / transfer data format
    scaler = preprocessing.MinMaxScaler()

    # Split the normal training data into training and validation sets (80% train, 20% validation)
    df_normal_train, df_normal_eval = train_test_split(df_normal_train, test_size=0.2, random_state=20)

    # Combine normal evaluation and attack evaluation data into a single DataFrame
    df_eval = pd.concat([df_normal_eval,df_attack_eval],axis=0)
    # Extract features (drop unnecessary columns) and labels from DataFrames
    ...
    # Scale the validation data using the fitted scaler
...
def train_data_processing_numpy(X_normal, X_attack_eval):
    # Split the normal data into training and evaluation sets
    X_normal_train, X_normal_eval = train_test_split(X_normal, test_size=0.2, random_state=20)

    # Create labels for the training and evaluation sets
    # All training data is assumed to be normal, so labels are 1s
    y_train = np.ones(X_normal_train.shape[0])
    
    # Create labels for normal and attack evaluation data
    y_eval_normal = np.ones(X_normal_eval.shape[0])
    y_eval_attack = np.zeros(df_attack_eval.shape[0])

    # Combine normal and attack evaluation data and labels
...
def pred(model, X):
    # Disable gradient calculation to optimize performance
    with torch.no_grad():
        # Set the model to evaluation mode
        model.eval()
        # Initialize a list to store predictions
        y = []
        # Iterate over the input data in batches of 16
        for i in range(0, X.shape[0], 16):
            # Extract a batch of data
            b_x = X[i:i + 16].cuda()
            # Forward pass through the model
            temp_x = model(b_x)
            # Make predictions using the model's specific prediction function
            temp_y = model.pred(temp_x, b_x)
            # Append the batch predictions to the overall prediction list
            y.extend(temp_y)
...
# Measures the throughput and memory usage of the model during inference.
def test_throughput(test_model, BATCH_SIZE, X):
    ...
    with torch.no_grad():
        ...
        # Initialize a counter for total processed samples
        total_ = 0
...
# Trains an autoencoder model for anomaly detection in network traffic data.
def train_autoencoder(model, df_normal_train, df_attack_eval, model_save_path, TWO_D):
    ...
    # prepocessing
    # Preprocess data based on its type (NumPy array or DataFrame)
    ...
    # Calculate FLOPs and MACs for the model (for performance analysis)
    with torch.no_grad():
        model.eval()
...
# Tests the Kitsune model on the given test data.
